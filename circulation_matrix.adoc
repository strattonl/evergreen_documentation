...
|Required to enable the rule; unchecking is the only way to "delete" a rule 
|Required; Patron profile the rule applies to; if a rule applies to all, select "Users" 
|Required; CONS to apply for all libraries, otherwise specify LIB code
|Circ will be limited to the checkout library only
|Circ will be limited to materials owned by the specific library
|Circ will be limited to the specific library's patrons
|The rule is ONLY for renewals 
|The circ is limited to juvenile patron accounts; juvenile is calculated by the date of birth field and the Juvenile Age Threshold Library Setting
|Indicates the materials the rule applies to; leave blank to apply the rule to all materials
|The rule will be applied only to materials within a specific library's copy location
|The rule applies based on the MARC 008 Type code in the bibliographic record 
